...
- `index.html` — homepage: candidate support counter + countdown. API in `worker.js` (Cloudflare Worker + Durable Object). Do not change the counter or existing content without an explicit request.
- `game/` — the game "61 — יש לך דקה", served at `/game/`. Full spec: `game61/docs/61_claude_product_spec_he.md`.
  - `game/index.html` — page + React app (UMD React + htm from CDN, no build). The app script is kept **inline** on purpose: Windows Defender flags it as `Trojan:Win32/MalUri.A!cl` (false positive) when saved as a separate `app.js`, which blocks local editing.
  - `game/engine.js` — pure game logic (no DOM), shared by the page and the tests.
  - `game/style.css`, `game/polls.json` (written by the poll updater; the engine has an embedded snapshot as fallback).
...
- **Blackout.** From `blackoutStart` to `blackoutEnd` (Israel time, `game61/poll-updater/config.json`) the updater writes `{"hidden": true}`; the client also hides poll mode from `ELECTION.pollsHiddenFrom` in `game/index.html`. Both dates must be verified against the election propaganda law before changing them.
...
- Share links point to the same challenge (hash routes `#/daily/YYYY-MM-DD`, `#/c/<seed>`). Do not claim personal dynamic OG cards without a server.
